...
(
	...
	na varchar, pa varchar,
	photos bytea, bir date,
...
begin
SELECT NOW()::timestamp into datatimes;
insert into stud.portfol(surname, name, patronymic, photo, birthday, address_reg, address_fact, "number", 
...
values (su, na, pa, photos, bir, ar, af, numbers, sn, mp, pasp,  ass, FIOm, wm, nm, FIOf, wf, nf, fam, 
...
CREATE OR REPLACE PROCEDURE func.updatedocidstud -- поменять студента у достижения
(
	id_docs int,
	id_studs int
)
...
begin
SELECT NOW()::timestamp into datatimes;
select stud.achi.id_stud from stud.achi where id_doc = id_docs limit 1 into id_st;
select stud.teach.id_login from stud.teach where teach.id_teach = --
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = 
(select stud.achi.id_stud from stud.achi where achi.id_doc = id_docs))) limit 1 into id_l;
update stud.achi set id_stud = id_studs where id_doc = id_docs;--
insert into prot.protocol(id_login, ivent, date) values (id_l, 'Updated id_stud doc with '||id_st||' to '||id_studs||'.', datatimes);--
end;
$$;
...
CREATE OR REPLACE PROCEDURE func.updateachiname --изменить имя у достижения
(
	id_docs int,
	names varchar
)
...
begin
SELECT NOW()::timestamp into datatimes;
select stud.achi.name from stud.achi where id_doc = id_docs limit 1 into namen;
select stud.teach.id_login from stud.teach where teach.id_teach = --
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = 
(select stud.achi.id_stud from stud.achi where doc.id_doc = id_docs))) limit 1 into id_l;
update stud.achi set name = names where id_doc = id_docs;--
insert into prot.protocol(id_login, ivent, date) values (id_l, 'Updated name doc with '||namen||' to '||names||'.', datatimes);--
end;
$$;
...
CREATE OR REPLACE PROCEDURE func.updatedocachi --изменить достижение
(
	id_docs int,
	docu bytea
)
...
begin
SELECT NOW()::timestamp into datatimes;
select stud.achi.documents from stud.achi where id_doc = id_docs limit 1 into d;
select stud.teach.id_login from stud.teach where teach.id_teach = --
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = 
(select stud.achi.id_stud from stud.achi where doc.id_doc = id_docs))) limit 1 into id_l;
update stud.achi set documents = docu where id_doc = id_docs;--
insert into prot.protocol(id_login, ivent, date) values (id_l, 'Updated documents doc with '||d||' to '||docu||'.', datatimes);--
end;
$$;
