...
CREATE OR REPLACE FUNCTION func.dow --путь к документу+
(
	id_d integer)
    RETURNS TABLE(p_path text) 
    LANGUAGE 'plpgsql'
AS 
$$
declare
...
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = 
(select stud.doc.id_stud from stud.doc where doc.id_doc = id_d))) limit 1 into id_l;
return query(select pathh from stud.doc where id_d = id_doc);
insert into prot.protocol(id_login, ivent, date) values (id_l, 'path doc with '||id_d||'.', datatimes);
end;
$$;

CREATE OR REPLACE FUNCTION func.dowachi --путь к достижению+
(
	id_d integer)
    RETURNS TABLE(p_path text) 
    LANGUAGE 'plpgsql'
...
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = 
(select stud.doc.id_stud from stud.doc where doc.id_doc = id_d))) limit 1 into id_l;
return query(select pathh from stud.achi where id_d = id_doc);
insert into prot.protocol(id_login, ivent, date) values (id_l, 'dowland doc with '||id_d||'.', datatimes);
end;
$$;
