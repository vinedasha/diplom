...
insert into stud.doc(id_stud, name) values (id_studs, names) returning id_doc into id_docs;
...
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = id_studs)) limit 1 into id_logins;
insert into stud.achi(id_stud, name) values (id_studs, names) returning id_doc into id_docs;
insert into prot.protocol(id_login, ivent, date) values (id_logins, 'add id document with '||id_docs||' and name '||names||'.', datatimes);
end;
$$;
...
CREATE OR REPLACE PROCEDURE func.updateportfolfio --изменить фамилию у студента+
(
	id_studs int,
	su varchar
)
...
CREATE OR REPLACE PROCEDURE func.updateportfolar --изменить адрес регестрации у студента+
(
	id_studs int,
	ar varchar
)
...
begin
SELECT NOW()::timestamp into datatimes;--
select stud.portfol.address_reg from stud.portfol where id_stud = id_studs limit 1 into adr;--
select stud.teach.id_login from stud.teach where teach.id_teach = --
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = id_studs)) limit 1 into id_l;
update stud.portfol set address_reg = ar where id_stud = id_studs;--
insert into prot.protocol(id_login, ivent, date) values (id_l, 'Updated address_reg stud with '||adr||' to '||ar||'.', datatimes);--
end;
$$;

CREATE OR REPLACE PROCEDURE func.updateportfolaf --изменить офф адрес у студента+
(
	id_studs int,
	af varchar
)
...
begin
SELECT NOW()::timestamp into datatimes;--
select stud.portfol.address_fact from stud.portfol where id_stud = id_studs limit 1 into adf;--
select stud.teach.id_login from stud.teach where teach.id_teach = --
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = id_studs)) limit 1 into id_l;
update stud.portfol set address_fact = af where id_stud = id_studs;--
insert into prot.protocol(id_login, ivent, date) values (id_l, 'Updated fio stud with '||adf||' to '||af||'.', datatimes);--
end;
$$;

CREATE OR REPLACE PROCEDURE func.updateportfolnum --изменить номер телефона у студента+
(
	id_studs int,
	num int
)
...
begin
SELECT NOW()::timestamp into datatimes;--
select stud.portfol.number from stud.portfol where id_stud = id_studs limit 1 into numb;--
select stud.teach.id_login from stud.teach where teach.id_teach = --
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = id_studs)) limit 1 into id_l;
update stud.portfol set number = num where id_stud = id_studs;--
insert into prot.protocol(id_login, ivent, date) values (id_l, 'Updated fio stud with '||numb||' to '||num||'.', datatimes);--
end;
$$;

CREATE OR REPLACE PROCEDURE func.updateportfolid_t --изменить препода у студента+
(
	id_studs int,
	id_teachs int
)
...
begin
SELECT NOW()::timestamp into datatimes;--
select stud.con.id_teach from stud.con where id_stud = id_studs limit 1 into id_t;--
select stud.teach.id_login from stud.teach where teach.id_teach = --
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = id_studs)) limit 1 into id_l;
update stud.con set id_teach = id_teachs where id_stud = id_studs;--
insert into prot.protocol(id_login, ivent, date) values (id_l, 'Updated id_t stud with '||id_t||' to '||id_teachs||'.', datatimes);--
end;
$$;
...
SELECT NOW()::timestamp into datatimes;
select stud.teach.id_login from stud.teach where id_teach = id_teachs limit 1 into id_l;--
return query(select stud.portfol.id_stud, stud.portfol.surname, stud.portfol.name, stud.portfol.patronymic from stud.portfol where portfol.id_stud in (select stud.con.id_stud from stud.con where con.id_teach = id_teachs) ORDER BY id_stud ASC );
...
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = id_studs)) limit 1 into id_l;--
return query(select stud.doc.name from stud.doc where id_stud = id_studs ORDER BY id_doc ASC );
...
(select stud.con.id_teach from stud.con where con.id_stud = 
(select stud.portfol.id_stud from stud.portfol where  portfol.id_stud = id_studs)) limit 1 into id_l;--
return query(select stud.achi.name from stud.achi where id_stud = id_studs ORDER BY id_doc ASC );
insert into prot.protocol(id_login, ivent, date) values (id_l, 'select doc stud with '||id_studs||'.', datatimes);
end;
$$;
